      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="pos-system" -c user.email="pos-system@example.com" add "${targetFile}"`);
            execSync(`git -c user.name="pos-system" -c user.email="pos-system@example.com" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
